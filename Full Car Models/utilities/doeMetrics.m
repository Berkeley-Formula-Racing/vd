...
for i = 1:n
    ...
    car = carCell{i,1};
    if isempty(car.comp) || ~isfield(car.comp.times,'autocross')
        continue
    end

    try
        rows(i) = extractCase(car,i);
        if runRamp
            R = rampSweep(car,rampOpts);
            details.ramp{i} = R;
            rows(i).understeer_gradient_10_deg_per_g = ...
                interpSafe(R.perSpeed.vCar,R.perSpeed.K_linear,10);
            rows(i).understeer_gradient_25_deg_per_g = ...
                interpSafe(R.perSpeed.vCar,R.perSpeed.K_linear,25);
            rows(i).rebalance_speed_mps = zeroCrossing( ...
                R.perSpeed.vCar,R.perSpeed.K_linear);
        end
    catch ME
        rows(i).valid = false;
        rows(i).error_message = string(ME.message);
    end
end
...
function r = extractCase(car,idx)
r = blankRow();
r.case_index = idx;
c = car.comp;
