...
names = { ...
    ...
    'roll_gradient_deg_per_g','rear_roll_camber_outer_deg_per_deg', ...
    'rear_roll_camber_inner_deg_per_deg','ride_camber_front_deg_per_in', ...
    'ride_camber_rear_deg_per_in', ...
    'grip_scaling_front','grip_scaling_rear','I_wheel','I_driveline','Crr', ...
    'spring_rate_front_lb_in','spring_rate_rear_lb_in','motion_ratio_front', ...
    'motion_ratio_rear','static_front_ride_height_in', ...
    'static_rear_ride_height_in','map_reference_front_ride_height_in', ...
    'map_reference_rear_ride_height_in'};
...
values = { ...
    ...
    cP.roll_gradient_deg_per_g,cP.rear_roll_camber_outer_deg_per_deg, ...
    cP.rear_roll_camber_inner_deg_per_deg,cP.ride_camber_front_deg_per_in, ...
    cP.ride_camber_rear_deg_per_in, ...
    tP.grip_scaling_front,tP.grip_scaling_rear,cP.I_wheel,cP.I_driveline,cP.Crr, ...
    cP.spring_rate_front_lb_in,cP.spring_rate_rear_lb_in,cP.motion_ratio_front, ...
    cP.motion_ratio_rear,cP.static_front_ride_height_in, ...
    cP.static_rear_ride_height_in,aP.map_reference_front_ride_height_in, ...
    aP.map_reference_rear_ride_height_in};
...
aeroMap = [];
if aeroMapEnabled(aP)
    if ~isfield(aP,'map_path') || ~isfile(aP.map_path)
        error('parameters_loop:aeroMapNotFound', ...
            'use_aeromap is true but aeroParams.map_path does not name a readable CSV file.')
    end
    aeroMap = AeroMap(aP.map_path);
end
...
for i = 1:numRuns
    ...
    aero = Aero(q.cda,q.cla,q.distribution,q.cla_p_deg_p,q.D_p_deg_p,aeroMap);
    ...
    tail = {powertrain,tire,q.ackermann,q.static_r_toe,q.grip_scaling_front, ...
        q.grip_scaling_rear,q.I_wheel,q.I_driveline,q.Crr,rideHeightAero(q), ...
        camberKinematics(q)};
...
function config = camberKinematics(q)
config = struct( ...
...
function config = rideHeightAero(q)
lbInToNpm = 175.126835;
config = struct( ...
    'enabled',true, ...
    'spring_rate_front_lb_in',q.spring_rate_front_lb_in, ...
    'spring_rate_rear_lb_in',q.spring_rate_rear_lb_in, ...
    'motion_ratio_front',q.motion_ratio_front, ...
    'motion_ratio_rear',q.motion_ratio_rear, ...
    'wheel_rate_front_Npm',q.spring_rate_front_lb_in*lbInToNpm*q.motion_ratio_front^2, ...
    'wheel_rate_rear_Npm',q.spring_rate_rear_lb_in*lbInToNpm*q.motion_ratio_rear^2, ...
    'static_front_ride_height_in',q.static_front_ride_height_in, ...
    'static_rear_ride_height_in',q.static_rear_ride_height_in, ...
