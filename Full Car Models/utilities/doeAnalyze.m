...
defaultResponses = {'modeled_dynamic_points','objective_score', ...
...
sobolSamples = getOr(opts,'sobolSamples',4096);
sobolSeed = getOr(opts,'sobolSeed',0);
sobolBootstrapSamples = getOr(opts,'sobolBootstrapSamples',200);
...
sobol = struct();
sobolAnalysis = struct('models',models,'gpModels',gpModels, ...
    'preferredModel',preferredModel,'predictorBounds',predictorBounds, ...
    'settings',struct('predictors',{predictors}, ...
    'sobolBootstrapSamples',sobolBootstrapSamples));
for i=1:numel(responses)
    response=responses{i};
    sobol.(response) = doeSobol(sobolAnalysis,response,sobolSamples,sobolSeed);
end
...
figures=plotDOEMetrics(S.designTable,metricTable, ...
    ...
    'sobol',sobol,'preferredModel',preferredModel,'validation',validation));
...
A=struct('metricTable',metricTable,'metricDetails',metricDetails, ...
    ...
    'sobolSamples',sobolSamples,'sobolSeed',sobolSeed, ...
    'sobolBootstrapSamples',sobolBootstrapSamples));
